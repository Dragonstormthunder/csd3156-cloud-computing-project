...
/*!
   * File Modified by Goh Jun Lin Wayne for the purposes of creating an Augmented Reality application
 */
/*
...
import com.google.ar.core.examples.kotlin.sofasogood.app.Vec3
import com.google.ar.core.examples.kotlin.sofasogood.app.listModelInformation
...
class HelloArRenderer(val activity: HelloArActivity) :
   ...
   */
  fun LoadAllModels(render : SampleRender) {
  ...
  @brief
    ...
    @param render
      the current renderer to render all objects
   */
  fun RenderAllModels(render : SampleRender)
  {
    for ((anchor, trackable, model ,scale,position,rotation) in
    wrappedAnchors.filter { it.anchor.trackingState == TrackingState.TRACKING }) {
      ...
      Matrix.rotateM(rotationMatrix,0, rotation.x, 1f,
        0f, 0f)
      Matrix.rotateM(rotationMatrix,0, rotation.z, 0f,
       0f, 1f)
      Matrix.rotateM(rotationMatrix,0, rotation.y, 0f,
        1f, 0f)
      ...
      render.draw(listModelInformation[model].Mesh, virtualObjectShader, virtualSceneFramebuffer)
    }
  }
  ...
  /**
   *  Currently not used because otherwise i need to find all the different types of maps
   *  if wanting to implement later then ill do it i guess
   *
   *
   */
  ...
  private fun handleTap(frame: Frame, camera: Camera) {
    ...
    if (firstHitResult != null) {
      ...
      wrappedAnchors.add(WrappedAnchor(firstHitResult.createAnchor(),
        firstHitResult.trackable,
        currentlySelectedInformation.modelIndex,
        currentlySelectedInformation.scale,
        currentlySelectedInformation.position,
        currentlySelectedInformation.rotation
      ))
 ...
 */
//This is basicallly a class that represents the object in the application
